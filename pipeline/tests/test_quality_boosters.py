...
async def test_challenge_problem_with_agent():
    ...
    """
    with patch("devils_advocate.generate_response_with_fallback", new=AsyncMock(return_value=mock_llm_json)):
...
async def test_detect_portfolio_blind_spots():
    ...
    """
    with patch("blind_spot_detector.generate_response_with_fallback", new=AsyncMock(return_value=mock_blind_json)):
